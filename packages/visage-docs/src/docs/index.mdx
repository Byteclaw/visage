...
If your application requires to be responsive then you need to use `ResponsiveDesignSystem` component instead.
...
As was already mentioned, in order to use Visage for responsive application you need to import [ResponsiveDesignSystem](./components/responsive-design-system) component. Under the hood, by default, this component uses [window.matchMedia](https://developer.mozilla.org/en-US/docs/Web/API/Window/matchMedia) API to match breakpoints and set up a current device breakpoint.
You don't need to worry about that, to start implementing responsive styles and behaviour.
